"""
    ...
    Methods:
        ...
        audio_to_binary(audio_data): This is actually turning Bytes data into binary string
        ...
        load_binary_data(file_path): Loads data from a file, return data stored in byte.
...
class OFDMTransmitter:
    ...
    def split_data_into_blocks(self, binary_data, block_size, prefix_length,fs, f_low, f_high):
        ...
        if use_ldpc:
            ...
            #using LDPC won't effect how we manipulate the first OFDM block
            if use_pilot_tone:
                symbols_extended=self.initialize_pilot_block(n_bins, 1)
                np.random.seed(None) 
            ...
            ldpc_encoded_length = (bits_per_block//24)*24#find the closest multiple of 24 for LDPC requirement
            ldpc_data_length = int(ldpc_encoded_length/2)#this is the new bits_per_block
            print("ldpc_encoded_length_for_1_block",ldpc_encoded_length)
            ...
            for i in range(num_blocks):
                ...
                #convert string to list,do the ldpc incoding
            ...
            if use_pilot_tone:
                #first append the pilot tone to blocks_with_prefix
                symbols_extended=self.initialize_pilot_block(n_bins, 1)
                np.random.seed(None) 
                time_domain_signal = self.inverse_dft(symbols_extended)
                transmitted_signal = self.add_cyclic_prefix(time_domain_signal, prefix_length)
            ...
            for i in range(num_blocks):
                ...
                # Add cyclic prefix
                transmitted_signal = self.add_cyclic_prefix(time_domain_signal, prefix_length)#a numby array
        ...
        concatenated_blocks = []
        for block in blocks_with_prefix:
            concatenated_blocks.extend(block)

        return np.array(concatenated_blocks)#turn the list to array
    
    def initialize_pilot_block(self, n_bins, seed):
    ...
    def audio_to_binary(self, audio_data):
        """Convert byte data to a binary string.this change the data from raw bytes to binary string."""
...
if __name__ == "__main__":
    ...
    data = transmitter.load_binary_data(transmitted_binary_path)# the data is in byte format now
    ...
    #if withouth header
    binary_data = transmitter.audio_to_binary(data) #turn data into binary string

    # Transmit the signal
    transmitted_signal = transmitter.transmit_signal(binary_data, block_size, prefix_length,fs, 996.09, 8583.98)
    ...
    generator = ChirpSignalGenerator(f_low=761.71875, f_high=8824.21875)
    generator.generate_chirp_signal()
    chirp_path = 'chirps/' + chirp_name + '.wav'
    generator.save_as_wav(chirp_path)

    # Load the chir p data from the saved file
